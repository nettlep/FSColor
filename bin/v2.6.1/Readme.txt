...
------------------------------------------------------------------------------------------------------------------------------------
  ...
  Copyright 2013, Fluid Studios & Paul Nettle. All rights reserved.
  
  Alpha support by Fausto Cardone (faustus@raylight.it)
...
2: Use the following line to your source code to import the DLL's one-and-only function:
	...
	extern "C" __declspec( dllimport ) bool WINAPI FSColorPickerDoModal(unsigned int * currentColor, const bool currentColorIsDefault, unsigned int * originalColor, const bool originalColorIsDefault, const int initialExpansionState);
...
3: Have a look at how to call it:
	...
	//
	// If a color is defaulted, it's input color value is ignored and the color picker will start
	// with its own defaults. On return, these colors will be set to what the user selected.
	//
	...
	unsigned int	currentColor = 0x00000000;
	bool           currentColorIsDefault = true;
	
	unsigned int	originalColor = 0x00000000;
	bool           originalColorIsDefault = true;

	bool	userClickedOK = FSColorPickerDoModal(&currentColor, currentColorIsDefault, &originalColor, originalColorIsDefault, 0);
